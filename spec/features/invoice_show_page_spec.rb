...
describe 'Viewing an invoice show page' do
  before do 
    ...
    @clean1 = create(:clean, location_id:@location.id, date: Date.today(), plot_id: @plot.id, completed: true, clean_type: "Pre-Paint" )
    @clean2 = create(:clean, location_id:@location.id, date: Date.today(), plot_id: @plot.id, completed:true, clean_type:"Variation Order")
    @clean3 = create(:clean, location_id:@location.id, date: Date.today(), plot_id: @plot.id, clean_type:"Demo" ) 
    @cost_house_location = CostHouseLocation.create( {house_id:@house.id, location_id:@location.id, pre_paint_cost: 100, post_paint_cost: 90, demo_cost: 90, sparkle_cost: 80 } )
    @extra = create(:extra)
